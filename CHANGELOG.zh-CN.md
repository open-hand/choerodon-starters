...
### 优化
- 优化liquibase初始化工具 支持指定文件地址
- 补充gitlab 缺失api

# [0.18.2]

### 新增
- 支持所有sagaTask执行失败，触发回调方法
- 新增文件分片模块，支持断点续传

### 修改
- 升级底层依赖为`hzero-starter-parent`到1.5.3版本
- 更新gitlab4j-api部分查询接口
...
- `choerodon-starter-asgard`，该依赖主要用于支持基于`saga`的数据最终一致性。。更多信息可以了解[choerodon-asgard](https://github.com/choerodon/choerodon-asgard)。

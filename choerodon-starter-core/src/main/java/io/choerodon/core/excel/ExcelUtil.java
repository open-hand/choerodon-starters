...
public class ExcelUtil {
        ...
        for (int cellNum = 0; cellNum < lastCellNum; cellNum++) {
            ...
            if (propertyMap.isEmpty()) {
                ...
                if (!propertyMap.keySet().contains(cellValue)) {
                    throw new CommonException(ExceptionConstants.ExcelErrorCode.EXCEL_PROPERTYMAP_NOT_CONTAIN_CELL, cellValue);
                }
    ...
    public static <T> T getObject(Row row, Class<T> clazz, Map<Integer, String> titleRow,
        ...
        for (int cellNum = 0; cellNum < lastCellNum; cellNum++) {
            ...
            if (property.contains("scheduleEntryTime") || property.contains("scheduleExitTime")) {
    ...
    private static void setHeaderStyle(CellStyle headerStyle, Workbook book) {
        ...
        font.setBold(true);   //变粗
        font.setColor(HSSFColor.HSSFColorPredefined.BLUE.getIndex());   //蓝色
    ...
    public static byte[] getBytes(Workbook workbook) {
        try (ByteArrayOutputStream workbookOutputStream = new ByteArrayOutputStream()) {
            ...
            throw new CommonException(ExceptionConstants.ExcelErrorCode.EXCEL_WORKBOOK_WRITE_OUTPUT_STREAM, e);
        }
    }

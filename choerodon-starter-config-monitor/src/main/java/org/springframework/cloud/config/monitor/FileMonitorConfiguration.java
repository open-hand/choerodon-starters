...
public class FileMonitorConfiguration implements SmartLifecycle, ResourceLoaderAware {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FileMonitorConfiguration.class);
    ...
    public synchronized void start() {
        if (!this.running) {
            ...
            if (this.directory != null && !this.directory.isEmpty()) {
                LOGGER.info("Monitoring for local config changes: {}", this.directory);
                try {
                    ...
                    LOGGER.debug("error.watcher.IOException {}", e.toString());
                }
            } else {
                LOGGER.info("Not monitoring for local config changes");
            }
            this.running = true;
        }
    }
        ...
        if (this.running) {
            if (this.watcher != null) {
                try {
                    ...
                    LOGGER.error("Failed to close watcher for " + this.directory.toString(),
                            e);
                }
            }
            this.running = false;
    ...
    private Set<Path> getFileRepo() {
        if (this.scmRepository != null) {
            try {
                ...
                LOGGER.error("Cannot resolve URI for path: " + this.scmRepository.getUri());
            }
        }
            ...
            for (String path : this.nativeEnvironmentRepository.getSearchLocations()) {
                ...
                if (resource.exists()) {
                    try {
                        ...
                        LOGGER.error("Cannot resolve URI for path: " + path);
                    }
                }
            }
            return paths;
        ...
        while (key != null) {
            for (WatchEvent<?> event : key.pollEvents()) {
                if (event.kind() == StandardWatchEventKinds.ENTRY_CREATE
                    ...
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("Watch Event: " + event.kind() + ": context: "
                                + event.context());
                    }
